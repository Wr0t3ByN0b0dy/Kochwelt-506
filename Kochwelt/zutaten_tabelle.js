...
let ingredientsForPierogi = ['g Weizenmehl', 'Ei', 'Prisen Salz für den Nudelteig', 'g Kartoffeln, mehlig kochend', 'Salz', 'g Schichtkäse', 'Zwiebel', 'g Butter', 'Pfeffer', 'Zweige Petersilie', 'g Schmand', 'Mehl, für die Arbeitsfläche' ]
...
function IngredientPierogi(event){
  ...
  for (let index = 0; index < ingredientsForPierogi.length; index++) {
    ...
    if (ingredientsForPierogi[index].startsWith ('g')) {
      ...
      tabelle.innerHTML += `<tr><td>${newAmount + ingredientsForPierogi[index]}</td></tr>`;
    } else {
      let newAmount = amountForPierogi[index] * multiplier / 4;
      tabelle.innerHTML += `<tr><td>${newAmount + " " + ingredientsForPierogi[index]}</td></tr>`;
    }
...
let ingredientsForSpaghetti = ['g Spaghetti oder Tortellini', 'g Schinken', 'Eigelb', 'g Butter', 'Prise Salz und Pfeffer', 'Prise Muskat', 'Parmesan']
let amountForSpaghetti = [100, 50, 1, 12.5, 1, 0.25, 1]

function IngredientSpaghetti(event){
  event.preventDefault()
  
  let tabelle = document.getElementById("number-of-ingredient-for-spaghetti");
  let input = document.getElementById("amountInput");
  let multiplier = input.value;
  ...
  for (let index = 0; index < ingredientsForSpaghetti.length; index++) {
    ...
    if (ingredientsForSpaghetti[index].startsWith ('g')) {
      let newAmount = amountForSpaghetti[index] * multiplier;
      tabelle.innerHTML += `<tr><td>${newAmount + ingredientsForSpaghetti[index]}</td></tr>`;
    } else {
      let newAmount = amountForSpaghetti[index] * multiplier;
      tabelle.innerHTML += `<tr><td>${newAmount + " " + ingredientsForSpaghetti[index]}</td></tr>`;
    }
...
let ingredientsForSalad = ['Salatgurke(n)', 'Paprikaschot(n), rot und grün', 'g Tomate(n)', 'Zwiebel(n)', 'g Schafskäse(n)', 'Glas Oliven, schwarze, ca 100g', 'Salz und Pfeffer', 'Zitrone(n)', 'ml Olivenöl', 'Oregano']
...
function IngredientSalad(event){
  ...
  for (let index = 0; index < ingredientsForSalad.length; index++) {
    ...
    if (ingredientsForSalad[index].startsWith ('g') || ingredientsForSalad[index].startsWith ('ml')) {
      let newAmount = amountForSalad[index] * multiplier / 4;
      tabelle.innerHTML += `<tr><td>${newAmount + ingredientsForSalad[index]}</td></tr>`;
    } else {
      let newAmount = amountForSalad[index] * multiplier / 4;
      tabelle.innerHTML += `<tr><td>${newAmount + " " + ingredientsForSalad[index]}</td></tr>`;
    }
...
let ingredientsForGulasch = ['g Rindergulsch', 'g Zwiebeln', 'ml Rinderfond', 'g Karotten', 'Stück Paprikaschote(n)', 'Stück Knoblauchzehe', 'g Butterschmalz', 'Teelöffel Tomatenmark', 'Teelöffel Paprikapulver edelsüß', 'Petersilie frisch und glatt', 'Spritzer Zitrone', 'Majoran, Kümmel ganz, Loorbeerblätter, Salz und Pfeffer nach Bedarf']
let amountForGulasch = [125, 75, 300, 38, 0.5, 0.25, 12.5, 1, 1, null, null, null]
...
function IngredientGulasch(event){
  ...
  for (let index = 0; index < ingredientsForGulasch.length; index++) {
    ...
    if (ingredientsForGulasch[index].startsWith ('g') || ingredientsForGulasch[index].startsWith ('ml')) {
      let newAmount = amountForGulasch[index] * multiplier;
      tabelle.innerHTML += `<tr><td>${newAmount + ingredientsForGulasch[index]}</td></tr>`;
    } else {
      let newAmount = amountForGulasch[index] * multiplier;
      tabelle.innerHTML += `<tr><td>${newAmount + " " + ingredientsForGulasch[index]}</td></tr>`;
    }
